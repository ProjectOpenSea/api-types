...
`@opensea/api-types` is TypeScript type declarations generated from the OpenSea API OpenAPI spec. There is no runtime code here, so a finding in this repository is usually a type that misdescribes what the API actually returns and walks a consumer into unsafe handling. Vulnerabilities in the API itself are a matter for the Bugcrowd brief's own scope, not for this repository.
